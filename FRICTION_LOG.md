...
**2026-09-13 - The CAMUS Info EF is not the EF of the distributed contours.**
Simpson's biplane EF computed from the ground-truth masks of all 500 patients
sits a constant 7.8 points above the `EF:` value in the Info files (r 0.977,
slope 0.99, limits of agreement +2.8 to +12.8; the same offset with the
area-length formula, so it is not the disc geometry). The paper computes its
clinical metrics against EF from the annotations of expert O1a (Table IV
caption) and reports inter-observer EF biases of the same size, so the Info
value is a different measurement, presumably the clinical one from the
scanner software. Anyone using the Info EF as the reference for mask-derived
EF inherits this offset. Dataset documentation issue, not a library bug.
